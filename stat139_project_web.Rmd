...
knitr::opts_chunk$set(echo = F, cache = T, message = F, warning = F,
                      fig.height = 5, fig.align = 'center', fig.pos = "H")
...
```{r corplot, fig.cap='Correlation plot of predictors and response.', fig.height = 4}
...
```{r state, fig.height = 6, fig.cap='Plotting predictors and response by state. and region'}
...
```{r hist, fig.cap='Histograms and boxplots of overdose deaths and log-transformed overdose deaths.'}
...
```{r reg, fig.cap = 'Linear regression of log-overdose deaths with unemployment rate.'}
...
```{r polynomial, fig.cap = 'Standardized residuals of the quadratic full-effects model appears to satisfy Normality assumptions.'}
...
```{r lasso, fig.cap='LASSO feature selection plotted against values of log-lambda.'}
...
```{r rf, fig.cap = 'Random forest feature importance.'}
# random forest
