...
>>>>>>> 64c9766c638a20f2e0bab606c5c0edee15ef8e22
lm1.train <- rmse(predict(lm1, train), train$overdoseDeaths)
lm1.test <- rmse(predict(lm1, test), test$overdoseDeaths)

# log linear model
summary(lm2 <- lm(log(overdoseDeaths) ~ unemployment, data = train))
lm2.train <- rmse(exp(predict(lm2, train)), train$overdoseDeaths)
lm2.test <- rmse(exp(predict(lm2, test)), test$overdoseDeaths)
```

The simple regression model has a positive coefficient for unemployment ($0.086897$). With a $t$-statistic of $9.56$ ($p$-value $\approx 0$), this coefficient is very significant. The model has a positive association between unemployment and overdose deaths. 
...
plot(log(overdoseDeaths) ~ unemployment, data = overdose,
     ...
     xlab = "Unemployment Percent", ylab = "Log Overdose Deaths",
     col = rgb(0, 0, 0, 0.3), pch = 16)
y <- predict(lm2, newdata = data.frame(unemployment = x))
lines(y ~ x, col = "red", lwd = 3)
```
...
# mixed effects model
lmer1 <- lmer(overdoseDeaths ~ dates + (1 | dates) + (1 + unemployment | state), data = train)

lmer1.train <- rmse(predict(lmer1, newdata = train), train$overdoseDeaths)
lmer1.test <- rmse(predict(lmer1, newdata = test), test$overdoseDeaths)
```
...
=======
  train = c(lm1.train, lm2.train, poly1.train, poly2.train, lasso.train, 
            rf1.train, lmer1.train),
  test = c(lm1.test, lm2.test, poly1.test, poly2.test, lasso.test, 
           rf1.test, lmer1.test),
  row.names = c("lm1", "lm2", "poly1", "poly2", "lasso", "rf1", "lmer1")
>>>>>>> 64c9766c638a20f2e0bab606c5c0edee15ef8e22
)
```
